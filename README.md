<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>.: Be The Hero :.</title>
  <style>
    body{
      font-family: 'Roboto', sans-serif;
    }
    .icon-ul{
      list-style: none;
      display: flex;
      /* justify-content: center; */
      align-items: flex-end;
    }
    .icon-li{
      align-items: center;
      text-align: center;
    }
    .title h1{
      text-transform: capitalize;
    }
    .wrapper-modules img + img {
      margin-left: 50px;
    }
  </style>
  <link href="https://fonts.googleapis.com/css?family=Roboto&display=swap" rel="stylesheet">
</head>
<body>
  <center>
    <img src="frontend/src/assets/logo.svg" alt="Be The Hero">
  </center>
  <div class="wrapper-objetivo">
    <div class="title">
      <h1>Objetivo</h1>
    </div>
    <div class="wrapper-content">
      <p>Conectar Organizações não governamentais e sem fins lucrativos a pessoas com uma disposição a ajudar!</p>
    </div>
    <div class="title">
      <h1>Features</h1>
    </div>  
    <div class="wrapper-features">
      <ul>
        <li>Cadastro de ONGs;</li>
        <li>Aplicação Web para cadastro de Casos;</li>
        <li>Aplicação Mobile para divulgação de casos;</li>
        <li>Notificações por e-mail através do App Mobile;</li>
        <li>Notificações por Whatsapp através do App Mobile.</li>
      </ul>
    </div>
    <div class="title">
      <h1>Stack</h1>
    </div>
    <div class="wrapper-Stack">
      <ul class="icon-ul">
        <li class="icon-li"><img src="https://i0.wp.com/codigosimples.net/wp-content/uploads/2017/03/nodejs.png?fit=500%2C500&ssl=1" width="200" heigh="200" alt="NodeJs"> <h2>Node.js</h2> </li>
        <li class="icon-li"><img src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a7/React-icon.svg/1200px-React-icon.svg.png" width="300" heigh="300" alt="React"> <h2>React</h2> </li>
      </ul>
    </div>
    <div class="title">
      <h1>modules</h1>
    </div>
    <div class="wrapper-modules">
      <img width="50" height="auto" src="data:image/png;base64," alt="Expo">
      <img width="50" height="auto" src="https://reactnavigation.org/img/spiro.svg" alt="React Navigation">
      <img width="200" height="auto" src="https://expressjs.com/images/express-facebook-share.png" alt="Express">

    </div>
  </div>
</body>
</html>